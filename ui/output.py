"""Output formatters for the VyOS Policy Matcher."""
...
def format_result(result: MatchResult, output_format: str = "table") -> str:
    ...
    Args:
        result: The match result from the matching engine.
...
def _format_table(result: MatchResult) -> str:
    ...
    if result.is_state_policy:
        lines.append(f"  Match Type:   Global State Policy")
        ...
        lines.append(f"  Match Type:   Default Action (no rule matched)")
        lines.append(f"  Action:       {_colorize_action(result.action)}")
    else:
        lines.append(f"  Chain:        {result.chain_family} {result.chain_name}")
...
def _append_rule_details(lines: list[str], rule: Any) -> None:
...
def _rule_to_dict(rule: Any) -> dict[str, Any]:
    """Convert a FirewallRule to a JSON-serializable dict."""
    d: dict[str, Any] = {
        "number": rule.number,
        "action": rule.action,
    }
